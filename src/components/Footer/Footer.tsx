...
const itemsNameInFooter: itemsFooter[] = [
...

	{
		...
		name: (
			<p>
				<span>8(800)123-45-67</span> оптовые продажи
			</p>
		),
	...
	},
	{
		...
		name: (
			<p>
				<span>8(800)123-45-68</span> розничные продажи
			</p>
		),
	...
	return (
		<footer className={'wrapper_footer'}>
			<div className={'columnInFooter'}>
				<h3>Компания</h3>
				{itemsNameInFooter.map(
					(item, index) => item.block === FooterBlocks.Company && <a key={index}>{item.name}</a>
				)}
			</div>
			<div className={'columnInFooter'}>
				<h3>Клиенту</h3>
				{itemsNameInFooter.map(
					(item, index) => item.block === FooterBlocks.ToClient && <a key={index}>{item.name}</a>
				)}
			</div>
			<div className={'columnInFooter'}>
				<h3>Контакты</h3>
				{itemsNameInFooter.map(
					(item, index) =>
						item.block === FooterBlocks.Contacts && (
