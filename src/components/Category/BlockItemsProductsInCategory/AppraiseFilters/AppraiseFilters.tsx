...
export function AppraiseFilters(props: AppraiseFilterAdaptiveProps) {
	...
	const cssVariables = useMemo(() => {
		...
		const progress = rangeValues.slice(0)
			.sort((a, b) => a - b)
	...
	return (
			<DivWithOutsideClick
				className={cn(css.root, isOpen && css.isOpen)}
			...
			>
				...
				<div className={css.wrapper} style={cssVariables}>
					...
					<div className={css.closeButton} onClick={onButtonClick}>×</div>
					<Button styleType={'blue'} onClick={() => props.handleApplyFilter(fromTo)}>Применить</Button>
				</div>
